...
-- 1. Player Profiles (keyed by Clerk user id)
--
-- Identity comes from Clerk, so there is no auth.users row to reference and no
-- auth.uid() to police access with. These rows are reached only by route
-- handlers using the service-role key, after Clerk has said who is calling:
-- RLS is on with no policies, so the public anon key cannot touch this table
-- even if it leaks.
...
CREATE TABLE IF NOT EXISTS public.player_profiles (
  ...
  CONSTRAINT player_profiles_tag_shape CHECK (tag ~ '^[A-Za-z0-9_]{3,16}$')
);

-- One tag per person, compared without case: "Chirag" and "chirag" collide.
-- A unique index is what actually enforces this — two people picking the same
-- tag at the same moment both pass a SELECT check, and only the index stops
-- the second write.
CREATE UNIQUE INDEX IF NOT EXISTS player_profiles_tag_unique
  ON public.player_profiles (lower(tag));
...
-- Enable RLS for library. As with player_profiles there are no policies: the
-- browser never reads or writes this table directly, only /api/user/library
-- does, after Clerk has identified the caller.
ALTER TABLE public.user_games ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view their own library entries" ON public.user_games;
DROP POLICY IF EXISTS "Users can insert their own library entries" ON public.user_games;
DROP POLICY IF EXISTS "Users can delete their own library entries" ON public.user_games;
...
CREATE POLICY "Sessions can be finalised once, while recent" ON public.game_analytics
  ...
  WITH CHECK (
    -- A finalised row must carry a bounded score and a usable name: either
    -- the arcade initials a signed-out player types, or the tag on their
    -- account. The ceiling comes from the game itself, so a bypass that talks
    -- to Supabase directly is still capped per game, not just at a global max.
    score IS NOT NULL
    AND score >= 0
    AND score <= (SELECT g.max_score FROM public.games g WHERE g.id = game_id)
    AND player_name ~ '^[A-Za-z0-9_]{1,16}$'
    AND completed = true
  );
...
VALUES
  ...
  (19, 'Pen Fight', 'pen-fight', 'The school-desk classic in full 3D. Flick your pens into the other side''s and knock them off the edge. Seven pens, each with its own weight, speed and grip. Play with AI, hand the phone back and forth with a friend beside you, or send a room code to a friend far away.', 'Action', 'Medium', 4.9, 0, NULL, NULL, '{"modes":"Play with AI · 2 friends, 1 phone · Friend far away","flick":"Drag back from your pen, release","aim":"Left / Right","power":"Up / Down","flick (keys)":"Space","switch pen":"Shift, or tap another pen","pause":"P or Esc"}'::jsonb, '["A pen whose centre crosses the desk edge falls off. Knock theirs off; keep yours on.","Knocking off a pen scores 100, and each extra pen in the same flick scores 50 more.","Three ways to play: with AI, two friends on one phone, or a friend far away on a room code.","With AI: clear its pens to win the round and earn a bonus. Lose all of yours and it is over.","Every pen is different — heavy pens shove, light pens fly, grippy pens stop short."]'::jsonb, 100000)
